<script lang="ts">
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		let newSeed = element.seed.value;

		let response;

		if (edit) {
			// Update the url via the API
			response = await fetch(`${API_URL}/update-seed-url-seed`, {
				method: 'POST',
				headers: {
			...
			response = await fetch(`${API_URL}/add-seed-to-url`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Authorization: `Bearer ${localStorage.getItem('token')}`
		...
		// Get the json response for debugging
		console.log(await response.json());

		// Dispatch a close signal to the parent
		dispatch('close');
	}
